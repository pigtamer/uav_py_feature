...
while(True):
    ...
    X_PATCH_NUM_MAX = int(frame.shape[0] / 40)
    Y_PATCH_NUM_MAX = int(frame.shape[1] / 40)

    ech_frame = [] # container for pathces on each frame
    for x_0 in range(int(frame.shape[0] / 40)):
        x_1 = x_0 + 40
        for y_0 in range(int(frame.shape[1] / 40)):
            y_1 = y_0 + 40
    ...
    v1 = ( 40*int(idx % (X_PATCH_NUM_MAX + 1)), 40*int(idx / (X_PATCH_NUM_MAX + 1)))
    v2 = ( 40*int(idx % (X_PATCH_NUM_MAX + 1)) + 40, 40*int(idx / (X_PATCH_NUM_MAX + 1)) + 40)
